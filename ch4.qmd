...
<i>You also can add axis titles by using `xlab()` and `ylab()`. Click to see example.</i>
...
::: {.callout-tip collapse="true"}
## Customising Invidual Axis

<i>You could also employ a combination of `axis.title` and `axis.title.y`, as `axis.title.x` inherits values from `axis.title`. Expand to See the example below:</i>
...
<i>Using `coord_cartesian(xlim = c(0, NA), ylim = c(0, NA))` will produce the same result. CLICK to See the example below:</i>
