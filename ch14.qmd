...
::: {.callout-note}
## Using `orientation = "y"`
<i>Since `{ggplot2}` version 3.0.0 it is also possible to draw geom's horizontally via the argument `orientation = "y"`. Expand to see example.</i>
...
::: {.callout-note}
Note that this will only work for continuous data. If you want to reverse discrete data, use the `fct_rev()` function from the [`{forcats}` package](https://forcats.tidyverse.org/). Expand to see example.</i>
